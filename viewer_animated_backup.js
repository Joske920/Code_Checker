...
class CNCViewer {
    constructor(containerId) {
        ...
        this.radiusCompensation = 'G40'; // G40=off, G41=G42=right
        this.toolpathPoints = [];
        this.rapidPoints = [];
    ...
    animateNextStep() {
        ...
        // Bepaal of deze stap smooth geanimeerd moet worden
        // EXCLUSIEF: Tool changes, offset changes, transformaties (TRANS, AROT, TRAFOOF)
        // INCLUSIEF: Alleen werkelijke bewegingen (G0, G1, G2, G3)
        // 
        // Reden: Transformaties zijn instantaan en veranderen coördinatenstelsels
        //        Deze mogen NIET geïnterpoleerd worden tussen from/to posities
        if (step.from && step.to && 
            step.command !== 'TOOL_CHANGE' && 
            step.command !== 'OFFSET_CHANGE' && 
            step.command !== 'TRANS' && 
            !step.isTranslation &&
            !step.isRotation &&
            !step.isReset) {
            
            // STAP 1: Teken de volledige toolpath lijn ZONDER tool te positioneren
            // skipToolUpdate=true voorkomt dat tool naar eindpositie springt (flikkeren)
            this.renderUpToStep(this.currentStepIndex, true);
            
            // STAP 2: Animeer de tool smooth van start naar eind positie
            // Tool wordt 60fps geïnterpoleerd, toolpath blijft zichtbaar
            this.animateSmoothMovement(step, delay, () => {
                this.currentStepIndex++;
                setTimeout(() => this.animateNextStep(), 0);
            });
        } else {
            // Niet-bewegingsstappen (tool wissel, rotatie, TRANS, etc.)
            // Direct renderen MET tool positie update (geen animatie nodig)
    ...
    updateToolPosition() {
        ...
        if (this.radiusCompensation !== 'G40' && this.currentTool && this.currentTool.radius) {
            ...
            // Find the current step to check if we're on an arc or line
            const currentStep = this.animationSteps[this.currentStepIndex];
            
            if (currentStep && currentStep.arcData) {
                // Arc compensation
    ...
    renderUpToStep(stepIndex, skipToolUpdate = false) {
        ...
        if (this.rotationHelper && !skipToolUpdate) {
            ...
            if (this.radiusCompensation !== 'G40' && this.currentTool && this.currentTool.radius) {
                ...
                // Check if current position is on an arc
